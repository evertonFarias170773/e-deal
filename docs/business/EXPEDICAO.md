...
- Pedido de **teste encerrado** (`propostas.encerrado_teste_em` preenchida) não entra no painel: `.is("encerrado_teste_em", null)` no mesmo `select` do universo. Corte independente do auto-ocultar de `ENTREGUE` acima — um trata de pedido que nunca foi real, o outro de pedido real que já terminou. O item **"Encerrar teste"** está no menu de ações da linha (permissão `propostas.encerrar_teste`, só Super Administrador); **reabrir só em Orçamentos**, onde o pedido continua visível com badge. Regra completa: `PEDIDOS-PRODUCAO.md` §8-A.
...
- **Liberação de ADM:** `expedicao.admin`, conferida dentro de `exp_liberar_despacho` (SECURITY DEFINER). Grava quem liberou (`auth.uid`), nome, quando e motivo obrigatório em `expedicao_despacho_liberacoes`; é de **uso único**, consumida pelo despacho, e pode ser cancelada antes (`exp_revogar_despacho`). É **outra** liberação que não a de recotação.
- **Recotar é livre (24/09/2026, mais tarde):** o expedidor recota sozinho, sem liberação prévia — a rota `cotar` deixou de exigir a liberação. **Aplicar** a recotação na proposta continua exigindo a liberação do admin (`expedicao_recotacao_liberacoes`, menu Ações → "Liberar aplicação da recotação"), conferida na rota `aplicar` e consumida por `exp_aplicar_recotacao`, sem mudança no banco. No modal, o ADM vê o campo de motivo e "Liberar despacho (ADM)" no painel da trava.
...
**FOB e RETIRA valem zero.** Em FOB o cliente contrata e paga o transporte; em RETIRA não há transporte nenhum — a mercadoria é buscada no balcão. Nos dois não há frete a cobrar, qualquer que seja a cotação em tela. (RETIRA entrou em 04/09/2026, com a proposta 21699: cotada em CIF/SEDEX e trocada para RETIRA, ela seguiu cobrando os R$ 17,43 da cotação anterior, porque a aba Fretes esconde os cards mas não desmarca o que já estava escolhido. O predicado único é `modalidadeCobraFrete`.) A regra vive num ponto único — `src/features/orcamentos/lib/modalidade-frete.ts` — e é aplicada na **fronteira de consumo**, os três lugares que precisam concordar: o resumo da tela, o resumo do salvamento e o valor gravado em `propostas.valor_frete` e `cotacao_frete.valor`. Os valores cotados continuam visíveis na lista como referência; só a opção **escolhida** é zerada.
...
O default é seguro porque **CIF e "sem modalidade" produzem exatamente o mesmo dinheiro e o mesmo rótulo**: `valorFreteEfetivo` e `aplicarModalidadeNosFretes` só agem em FOB e em RETIRA, e `nomeTransporteEfetivo` devolve o serviço cotado para tudo que não é FOB. Quem zera o frete é FOB e RETIRA, nunca CIF. Seis asserções em `scripts/testes/modalidade-frete-rotulo.test.mts` fixam essa equivalência, para o dia em que CIF ganhar regra própria a quebra aparecer no teste e não na tela.
...
> A tabela de layout de 16/08/2026 acima descreve a versão anterior. O desenho
> vigente é o de 02/09/2026 — réguas horizontais em vez de caixas, PEDIDO em
> corpo grande, destinatário (nome, A/C, endereço, bairro, Fone), CEP,
> CIDADE/UF, FORMA DE ENVIO, OBSERVAÇÕES, REMETENTE completo e rodapé com QR
> do site, DATA DE ENVIO e VOLUME — documentado no cabeçalho de
