...
| `public.propostas` | `modalidade_frete` (`RETIRA` \| `FOB` \| `CIF`) e `id_transportadora_cliente` (FK → `clientes(id_cliente)`) — declarados pelo vendedor na aba Fretes do Orçamento | `INSERT`, `UPDATE` | `LIBERADO` | Modalidade comercial do frete = **quem paga**, ortogonal a `frete_escolhido`/`cotacao_frete.servico`, que dizem por onde vai. Colunas aditivas e nuláveis, com CHECK e FK próprios; nenhuma constraint pré-existente foi alterada e não houve backfill. **Gravadas só enquanto `status_interno` é `NOVO` ou `AGUARDANDO`** (formas compostas `.../ EM ARTE` contam): fora disso ficam FORA do `UPDATE` — travado significa "não altera", nunca "apaga". O motivo da trava é o trigger `trg_frete_sync_financeiro` de `cotacao_frete`, que com zero pagamentos força `status_interno = 'NOVO'` e rebaixaria um pedido já liberado. **Em FOB e em RETIRA o frete vale zero** (`modalidadeCobraFrete`; RETIRA desde 04/09/2026) e desce pelo `DELETE`+`INSERT` de `cotacao_frete` que o salvamento já fazia — sem caminho de escrita novo. | Alto | `select id_int, status_interno, modalidade_frete, id_transportadora_cliente, valor_frete from public.propostas where id_int = :id_int;` | `2026-08-18` / Modalidade no Orçamento | Escrita só por `saveProposta` (`orcamentos.service.ts`); regra num ponto único em `src/features/orcamentos/lib/modalidade-frete.ts`. **A Expedição apenas LÊ estas colunas** (pré-seleção do despacho e aviso de divergência) e nunca escreve em `propostas.modalidade_frete`. Pendência conhecida: `copiar_proposta_v2` e `duplicar_proposta` não copiam estas colunas — duplicar proposta FOB perde a modalidade em silêncio. Migration: `supabase/migrations/20260818_propostas_modalidade_frete.sql`. Regra de negócio em `docs/business/EXPEDICAO.md` §5.2. |
...
| `public.pagamentos_v2` | `status`, `motivo_cancela` — exclusivamente para o cancelamento de cobrança já paga | `UPDATE` | `LIBERADO` | Exceção autorizada e nomeada ao bloqueio geral de cobrança paga (ver `business/CANCELAMENTO-COBRANCAS.md`), restrita a super admin pela rota `POST /api/cobrancas/cancelar-pago`. Não é escrita genérica: exige motivo de catálogo e destino definido para o valor. | Crítico | `select id, status, motivo_cancela, paid_at, data_confirmacao from public.pagamentos_v2 where id = :id;` | `2026-08-11` / Exceção super admin (cancelar-pago) | Sem chamada a provedor externo. Bloqueado quando a proposta está em status operacional (exceto `REVISAO ATENDENTE`); exige confirmação explícita para confirmação de mês anterior; crédito lançado via `mc_ajuste_avulso_criar` ANTES do cancelamento (falha na conta corrente impede a gravação). A rota `cancelar-externo` continua bloqueando cobrança recebida sem exceção. `cancelar-boleto` foi apagada em 26/08/2026 (órfã e divergente) e não presta mais garantia nenhuma. |
