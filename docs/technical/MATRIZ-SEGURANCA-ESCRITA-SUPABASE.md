...
> Fluxo público de avanço de status de produção via QR Code impresso na OS (página `/os`, sem login). Preparado conforme `PEDIDOS-PRODUCAO.md` seção 18 (rota tokenizada). Migration versionada **não aplicada** — ver `supabase/migrations/20260722_os_qr_producao.sql`. Feature flag `OS_QR_PUBLICO_ENABLED` desligada por padrão. Token opaco derivado por HMAC server-only (`OS_QR_TOKEN_SECRET`); o banco armazena somente hash SHA-256. Ordem única de bloqueio por OS: 1º `public.propostas` (`FOR UPDATE` por `id_int`), 2º `public.os_qr_tokens`.

| Tabela ou recurso | Campo ou escopo | Operações | Status | Motivo da decisão | Risco | Validação pós-gravação | Data/fase da liberação | Observações técnicas |
|---|---|---|---|---|---|---|---|---|
| `public.propostas` | `status_interno` (somente transições `EM PRODUCAO → EM IMPRESSAO → EM ACABAMENTO → EXPEDICAO`) | `UPDATE` | `FUTURO` | Avanço de etapa pelo chão de fábrica **somente** via RPC `os_qr_avancar` (SECURITY DEFINER): token válido + `FOR UPDATE` + checagem otimista do status esperado + matriz de transição embutida. | Crítico | Conferir `status_interno` por `id_int` + linha correspondente em `public.os_status_log`. | QR Produção v1 (preparada) | Nunca regride, nunca salta etapa, nunca atualiza ao abrir a página. Ator técnico `qr_producao` (sem usuário nominal na v1). |
...
| `public.os_status_log` | Trilha de tentativas e transições via QR (`status_anterior/novo`, `resultado`, `origem`, ator técnico) | `INSERT` | `FUTURO` | Auditoria estruturada exigida pelo FLUXO-OFICIAL §11, gravada exclusivamente pelas RPCs definer. | Alto | `select * from public.os_status_log where id_int = :id_int order by id desc;` | QR Produção v1 (preparada) | RLS habilitada **sem nenhuma policy**. Leitura administrativa futura terá gate próprio. |
