...
| `public.pagamentos_v2` | `cartao_provedor` | `INSERT`, `UPDATE` | `LIBERADO` | Identifica o adquirente do checkout de cartão (`C6` ou `ASAAS`). Sem ele, cobranças de provedores diferentes ficam indistinguíveis e o cancelamento aciona o provedor errado. Gravado pelo service oficial de criação de cobrança e lido pela rota de cancelamento externo. | Crítico | `select id, tipo_cobranca, cartao_provedor, cartao_checkout_id from public.pagamentos_v2 where id = :id;` | `2026-08-01` / Cartão Asaas | Coluna criada por `20260801_pagamentos_v2_cartao_provedor.sql`, que **precisa emitir GRANT de coluna explícito** — `20260721_conta_corrente_fase1a_aditiva.sql` trocou GRANT de tabela por GRANT coluna a coluna, então coluna nova nasce sem privilégio e derruba o INSERT. Não aceita dado de cartão nem credencial. `NULL` = legado anterior a 01/08/2026, lido como `C6`; não houve backfill. |
...
> Domínio da diferença financeira **após proposta paga**. Não confundir com `public.boletos` (Contas a Receber) nem com `public.pagamentos_v2` (pagamentos). Fase 1 **preparada, não aplicada** — ver `business/CONTA-CORRENTE-FASE-1-PREPARACAO.md` e `supabase/migrations/20260721_conta_corrente_fase1.sql`.

| Tabela | Campo | Operações | Status | Motivo da decisão | Risco | Validação pós-gravação | Data/fase da liberação | Observações técnicas |
|---|---|---|---|---|---|---|---|---|
| `public.conta_corrente_pendencias` | Estado da pendência (identidade, direção, motivo, valores, status) | `READ` | `FUTURO` | Fonte operacional do saldo da pendência. Leitura autenticada; escopo fino na aplicação. | Alto | `select id, id_int, id_cliente, direcao, valor_original, valor_saldo, valor_reservado, status from public.conta_corrente_pendencias where id_int = :id_int;` | Fase 1 CC (preparada) | Tabela nova. RLS habilitada, sem policy de escrita. |
| `public.conta_corrente_pendencias` | Alteração de saldo/estado | `INSERT`, `UPDATE` | `FUTURO` | Escrita **somente** via RPCs `cc_abrir_pendencia`/`cc_usar_pendencia`/`cc_encerrar_pendencia` (SECURITY DEFINER). Escrita direta de `anon`/`authenticated` revogada. | Crítico | Conferir `valor_saldo`/`valor_reservado`/`status` e a razão em `movimento_credito` após a operação. | Fase 1 CC (preparada) | Sem GRANT de INSERT/UPDATE a papéis da app. Uso único garantido por `FOR UPDATE`. |
...
| `public.movimento_credito` | Colunas estruturadas (`id_pendencia`, `tipo_evento`, `id_int_origem/destino`, `id_pagamento_destino`, `id_movimento_ref`, `motivo_evento`) | `READ` | `FUTURO` | Razão imutável de auditoria/reconciliação da Conta Corrente. | Alto | `select id, tipo_evento, tipo, valor, id_pendencia, id_movimento_ref from public.movimento_credito where id_pendencia = :id;` | Fase 1 CC (preparada) | Corrige a lacuna histórica: `movimento_credito` passa a constar nesta matriz. |
| `public.movimento_credito` | Lançamentos | `INSERT` | `FUTURO` | Inserção **somente** via RPCs. Escrita direta de `anon`/`authenticated` revogada; policy permissiva `geral` removida. | Crítico | Conferir o lançamento e a coerência com `valor_saldo` da pendência. | Fase 1 CC (preparada) — **acoplada à Fase 2** | Aplicar junto ao refactor das rotas legadas (usar-credito, resolver-diferenca, ajuste/estorno-credito, confirmar, pagamento-combinado). |
