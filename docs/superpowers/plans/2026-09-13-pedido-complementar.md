...
**Status:** **concluído em 14/09/2026** — E0 a E10 entregues e publicadas; plano encerrado. Aprovado pelo dono em 13/09/2026; decisões registradas nas seções 2 e 12.
...
- [x] **E6 — Rota aplicar-frete + RPC + card.** _(concluída em 14/09/2026; migration aplicada em produção como versão `20260914125714`; validada com Y = #22069: SEDEX aplicado pelo card, ledger id 1, cotação de Y com R$ 21,68 e 999 g, X intacto; a cobrança de Y para E8/E9 NÃO foi gerada, a pedido do dono Y segue em NOVO)_ Criar `supabase/migrations/20260915_complementar_aplicar_frete.sql`, `src/app/api/orcamentos/complementar/aplicar-frete/route.ts`, `FreteComplementarCard.tsx`; modificar `complementar.client.ts`, `OrcamentoFormPage.tsx` (aba Fretes → card). Depende de: E5. Validação: aplicar SEDEX → 1 linha no ledger com CHECKs satisfeitos; `cotacao_frete`: Y com `valor = a_cobrar` e `peso` = peso de Y, X com a mesma linha de antes (mesmo `id`); `propostas`: X inalterado, Y = subtotal + a_cobrar em NOVO; repetir a mesma chave → idempotente; `audit.logs_v2` só com UPDATEs em Y; `frete-status?idInt=Y` → `EM_DIA`; gerar cobrança de Y e confirmar (LIBERADO) para E8/E9.

- [x] **E7 — Guardas: cancelamento + cobrança.** _(concluída em 14/09/2026; validada: cancelar a #22067 recusa com `COMPLEMENTO_ABERTO`; complemento sem frete aplicado bloqueia a cobrança com `FRETE_COMPLEMENTAR_PENDENTE`; a #22069 com frete aplicado não bloqueia; a proposta comum #22072 cobra e cancela como antes. A desvinculação no cancelamento do complemento, guarda (d), fica para a E8, como o plano permite)_ `cancelar-proposta/route.ts`, `CancelPropostaModal.tsx`, `/api/propostas/frete-status/route.ts`, `frete-desatualizado.ts`. Depende de: E6 (a chamada da RPC de desvinculação no cancelamento do complemento pode ficar para E8). Validação: cancelar X → 409 `COMPLEMENTO_ABERTO`; complemento sem frete aplicado → aba Pagamentos bloqueia com "frete complementar pendente"; após aplicar, libera.

- [x] **E8 — Expedição: vínculo no painel + bloqueio + desvincular.** _(concluída em 14/09/2026; migration aplicada em produção como versão `20260914142842`; validada no painel com X = #22067, que foi despachada separada por Correios SEDEX e está EM TRANSITO, e Y = #22069, desvinculada; pedido comum #22066 provado com toda escrita abortada. X deixou de servir de principal: a E9 usa um par novo)_ Criar `supabase/migrations/20260915_desvincular_pedido_complementar.sql`; modificar `expedicao/types.ts`, `expedicao.service.ts`, `expedicao-acoes.service.ts` (gate + `desvincularComplementos`), `DespacharModal.tsx`, `KanbanTransportadoras.tsx`, `ExpedicaoPage.tsx`, `cancelar-proposta/route.ts`. Depende de: E6. Validação: X em EXPEDICAO com Y em produção → chip "+ compl. #Y"; Despachar recusa; com "Desvincular e despachar separado" + motivo → `id_int_pedido_principal` de Y = NULL, ledger carimbado, chat nos dois, `frete-status?idInt=Y` → `FRETE_COMPLEMENTAR_INVALIDADO`. Repetir com par novo para E9.

- [x] **E9 — Despacho conjunto + etiqueta + declaração.** _(concluída em 14/09/2026; sem migration; validada com X = #22098 e Y = #22099, despachados juntos por Correios SEDEX com rastreio fictício `E9TESTE22098BR`: mesma `data_despacho` nos dois, Y sem peso, volumes e rastreio em `expedicoes`, rastreio nos dois em `propostas_os`, os dois EM TRANSITO, uma linha só em `expedicoes` com o código, etiqueta com "#22098 + #22099" e declaração com os itens dos dois; negativo da decisão 5 por simulação no navegador, com a resposta de `cc__valor_pago` de Y trocada por zero e toda escrita abortada: `COMPLEMENTO_NAO_PAGO` e nada gravado)_ Criar `src/features/expedicao/lib/destino-despacho.ts`; modificar `expedicao-acoes.service.ts` (conjunto + repetição + propagação de coleta/entrega), `DespacharModal.tsx`, `etiqueta-viewmodel.service.ts` + template, `declaracao-viewmodel.service.ts`. Depende de: E8. Validação: X e Y em EXPEDICAO; despachar X (Correios, rastreio R) → `expedicoes`: mesma `data_despacho` nos dois, Y com `obs='Despachado junto com #X'` e sem peso/volumes/rastreio; `propostas_os.codigo_rastreamento` = R nos dois; `status_interno` EM TRANSITO nos dois; `os_status_log`; etiqueta imprime "Pedido #X + #Y"; declaração lista itens dos dois; `count(*) expedicoes where codigo_rastreamento='R'` = 1. Negativo da decisão 5: com Y em EXPEDICAO e uma cobrança não confirmada, despachar X → `COMPLEMENTO_NAO_PAGO`, nada gravado em `expedicoes` de X nem de Y.

- [x] **E10 — Docs finais + grant da permissão.** _(concluída em 14/09/2026; migration aplicada em produção como versão `20260914211942`, com guarda de 2 linhas: `propostas.complementar` acrescentada aos perfis 2 e 4, demais permissões e os outros 7 perfis intactos, `data_atualizacao` dos dois gravada pelo trigger `perfis_set_timestamp`, aceito pelo dono; docs finais publicados; validada na tela com o Vendedor de teste userteste1 vendo "Criar pedido complementar" na #22135, criada pelo app no cliente 58613 com vendedor userteste1 e paga por E-Amostra)_ Criar `supabase/migrations/20260916_perfis_propostas_complementar.sql` (grant a `perfis.id in (2, 4)` — Administrador e Vendedor, decisão 10); atualizar `docs/business/EXPEDICAO.md`, `FLUXO-OFICIAL-STATUS-PROPOSTAS.md`, `CONTA-CORRENTE-CREDITO.md`, `PEDIDO-COMPLEMENTAR.md`. Depende de: E9. Validação: `select id, nome, permissoes @> '["propostas.complementar"]' from perfis`; Vendedor vê o item do menu.
...
- 14/09/2026 — E10 publicada; **plano encerrado**. Seguem abertas, fora deste plano: a pendência fiscal de uma caixa com duas NF-e (decisão 7) e as limitações de `docs/business/PEDIDO-COMPLEMENTAR.md` §14.
