...
7. **Referências à rota apagada na Etapa 8.** `/api/cobrancas/cancelar-boleto` deixou de existir, e cinco lugares ainda falam dela como se existisse. Dois são documentação viva e afirmam garantia de segurança que a rota já não presta:
   - `docs/business/CANCELAMENTO-COBRANCAS.md:197` — "As rotas `cancelar-externo` e `cancelar-boleto` continuam recusando cobrança paga";
   - `docs/technical/MATRIZ-SEGURANCA-ESCRITA-SUPABASE.md:117` — mesma frase, na matriz de segurança;
   e três são comentários de código que citam a rota como referência de desenho: `src/app/api/cobrancas/cancelar-pago/route.ts:4`, `src/app/api/fiscal/emitir-nfe/route.ts:89`, `src/features/cobrancas/services/conta-corrente.service.ts:11`.
