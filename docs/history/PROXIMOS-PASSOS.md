...
1. confirmar se a pendência ainda existe;
2. consultar o código atual;
3. consultar a documentação oficial do domínio;
4. revisar a Matriz de Segurança;
5. obter autorização quando a mudança for sensível.

Itens já concluídos, substituídos ou incompatíveis com a arquitetura atual devem permanecer apenas como histórico.
...
- Atualizar `docs/technical/MATRIZ-SEGURANCA-ESCRITA-SUPABASE.md` sempre que uma fase de escrita avançar.
...
> **Nota:** Para o estado detalhado do Maestro V2 (Orçamentos, Roteador, Pendências e Testes Prioritários), consulte o arquivo `../maestro/STATUS-MAESTRO-V2.md`.

## Pendências Técnicas: Sincronização em Lote de Datas (Orçamentos)
...
O backlog atual deve ser confirmado no repositório, no planejamento vigente e nos documentos oficiais de cada módulo.
