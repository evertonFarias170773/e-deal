...
|---|---|---|---|---|---|---|---|---|
| `public.clientes` | Campos operacionais simples liberados (`obs`, `fantasia`, `telefone_fixo`, `whatsapp_1`, `whatsapp_2`, `email_contato`, `email`, `email_financeiro`, `site`) | `READ`, `UPDATE` | `LIBERADO` | `UPDATE` real expandido e validado no Supabase com payload restrito aos campos permitidos alterados. `Nome fantasia / Apelido` grava somente em `fantasia`; `E-mail principal` grava em `email_contato` e `email`; `apelido` permanece bloqueado. | Baixo/médio | `select id_cliente, fantasia, telefone_fixo, whatsapp_1, whatsapp_2, email_contato, email, email_financeiro, site, obs from public.clientes where id_cliente = :id_cliente;` | `2026-05-23` / Fase 1 expandida | Chave operacional: `id_cliente`. Sucesso só após confirmação real do Supabase e validação pós-gravação via `SELECT`. |
...
| `public.enderecos` | Campos operacionais de endereço | `UPDATE`, `INSERT` | `FUTURO` | Precisa de fase própria após estabilidade de `clientes`. | Médio | Conferir lista de endereços do cliente e endereço principal. | Futuro | Deve considerar endereço principal, entrega, cobrança e fiscal. |
...
| `public.clientes_socios` | Campos de vínculo comercial | `UPDATE`, `INSERT` | `FUTURO` | Deve entrar depois de `clientes` e `enderecos` estabilizados. | Médio/alto | Conferir vínculo exibido e relacionamento correto. | Futuro | Pode impactar proposta, autorização e relacionamento comercial. |
...
| `public.contatos` | Qualquer campo | `UPDATE`, `INSERT`, `DELETE` | `BLOQUEADO` | Continua bloqueado até o app usar Supabase `authenticated` de verdade. | Alto | Somente revisão de RLS e autenticação antes de liberar. | Futuro condicionado | Não liberar via `anon`. |
