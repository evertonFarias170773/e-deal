...
| Tabela | Campo | Operações | Status | Motivo da decisão | Risco | Validação pós-gravação | Data/fase da liberação | Observações técnicas |
|---|---|---|---|---|---|---|---|---|
| `public.pedidos` | `id_int`, `id_vendedor`, `id_cliente`, `status_pedido`, `status_pagamento`, `status_arte`, `status_producao`, `status_expedicao`, `descricao`, `valor_total`, `forma_pagamento`, `obs`, `data_pedido` | `INSERT` | `EM TESTE` | Registro controlado do pedido pai a partir da abertura de Boletim de Entrada (Abertura de OS) com validações severas de elegibilidade. | Médio | `select * from public.pedidos where id_int = :id_int;` | `2026-06-14` / Boletim OS | Risco médio; escrita restrita ao INSERT do registro pai, com `id_cliente` nulo. Sem cascade para modelos/artes nesta etapa. |
...
| `public.pedidos_modelos` | Qualquer campo | `UPDATE`, `INSERT`, `DELETE` | `BLOQUEADO` | Mapeamento e criação de lotes operacionais/modelos de OS serão implementados em etapa futura. | Alto | Não aplicável. | Permanente nesta etapa | Mantido bloqueado. |
