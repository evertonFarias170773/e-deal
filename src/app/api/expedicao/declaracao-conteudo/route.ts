...
import { montarDeclaracaoViewModel } from "@/features/expedicao/services/declaracao-viewmodel.service";
import { criarDeclaracaoElement } from "@/features/expedicao/pdf/DeclaracaoConteudoPdfDocument";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** Erro legível na aba: o PDF abre por navegação, igual à etiqueta. */
function respostaErro(request: Request, message: string, status: number) {
  ...
  return new NextResponse(
    `<!doctype html><meta charset="utf-8"><title>Declaração de conteúdo</title><body style="font-family:sans-serif;padding:2rem"><h1>Não foi possível gerar a declaração</h1><p>${escapado}</p></body>`,
...
export async function GET(request: Request) {
  ...
  if (!temPermissao) {
    return respostaErro(request, "Sem permissão para gerar a declaração (expedicao.view).", 403);
  }

  const vm = await montarDeclaracaoViewModel(supabase, idInt);
  if (!vm) return respostaErro(request, `Pedido #${idInt} não encontrado.`, 404);

  const pdf = await renderToBuffer(criarDeclaracaoElement(vm));
  return new NextResponse(new Uint8Array(pdf), {
    status: 200,
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `inline; filename="declaracao_conteudo_${idInt}.pdf"`,
