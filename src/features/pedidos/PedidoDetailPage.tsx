...
import { ArrowLeft, CreditCard, FileText, Package, Truck, MessageSquare, AlertCircle, RefreshCw, Scale, HardDrive, CheckCircle2, User, Phone, Mail, Calendar, MapPin, ClipboardList, Paperclip, Printer, Sparkles, Bookmark, QrCode, Barcode, Download, ExternalLink, Clock } from "lucide-react";
...
export function PedidoDetailPage({ idInt }: PedidoDetailPageProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-blue-200 bg-blue-50/70 p-4 text-blue-900 flex items-start gap-3">
        <AlertCircle className="h-5 w-5 text-blue-600 shrink-0 mt-0.5" />
        <div>
          <h4 className="font-bold text-sm">TELA OPERACIONAL DE DETALHES (PROTÓTIPO)</h4>
          <p className="text-xs text-blue-700 mt-1">
            Simulação das abas e tomadas de decisão. As alterações refletem no banco de dados local simulado.
          </p>
        </div>
      </div>
      ...
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap items-center gap-2">
            ...
            {pedido.urgente && (
              <span className="bg-red-100 dark:bg-red-950/30 text-red-700 dark:text-red-400 font-extrabold border border-red-200 dark:border-red-900 rounded-full px-2.5 py-1 text-xs">
                ⚡ URGENTE
              </span>
            )}
            <button
              onClick={() => toggleUrgente(idInt)}
              className="h-9 px-3 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50 text-xs font-semibold"
            >
              Toggle Urgência
            </button>
            <select
              ...
              className="h-9 px-3 rounded-xl border border-slate-200 text-xs font-semibold bg-white"
            >
      ...
      {/* Progresso Geral */}
      <section className="rounded-3xl border border-[#d7e5e8] bg-white dark:bg-slate-900 p-5 shadow-sm space-y-2">
      ...
      {/* Tabs */}
      <section className="rounded-3xl border border-[#d7e5e8] bg-white dark:bg-slate-900 p-2 shadow-sm">
        <div className="flex gap-2 overflow-x-auto font-sans">
          {(["resumo", "dados_comerciais", "produtos", "artes", "producao", "expedicao", "timeline"] as const).map((tab) => (
            <button
              key={tab}
              type="button"
              onClick={() => setActiveTab(tab)}
              className={`shrink-0 rounded-2xl px-4 py-2.5 text-xs font-bold transition uppercase tracking-wide ${
                activeTab === tab
      ...
      {/* Tab Content */}
      <section className="rounded-3xl border border-[#d7e5e8] bg-white dark:bg-slate-900 p-6 shadow-sm font-sans">
        {/* TAB RESUMO */}
        {activeTab === "resumo" && (
          <div className="space-y-6">
            {/* Header de Boletim */}
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="bg-slate-50 dark:bg-slate-950/20 border border-slate-100 dark:border-slate-800 p-5 rounded-3xl space-y-3">
              ...
              <div className="bg-slate-50 dark:bg-slate-950/20 border border-slate-100 dark:border-slate-800 p-5 rounded-3xl space-y-4">
                <h4 className="font-extrabold text-xs text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
              ...
              <div className="bg-slate-50 dark:bg-slate-950/20 border border-slate-100 dark:border-slate-800 p-5 rounded-3xl space-y-3 md:col-span-2">
                <h4 className="font-extrabold text-xs text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
        ...
        {activeTab === "dados_comerciais" && (
          <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              <div className="bg-slate-50 dark:bg-slate-950/20 border border-slate-100 dark:border-slate-800 p-5 rounded-3xl space-y-2">
            ...
            {(pedido.financialBlock || pedido.blockReason) && (
              <div className="bg-red-50/50 dark:bg-red-950/10 border border-red-200 dark:border-red-900 text-red-800 dark:text-red-300 p-5 rounded-3xl space-y-2">
                <h4 className="font-extrabold text-sm flex items-center gap-1.5">
                  <AlertCircle className="h-4 w-4 text-red-500" />
                  Bloqueios ou Restrições Ativas
                </h4>
                {pedido.financialBlock && (
                  <p className="text-xs">⚠️ <strong>Bloqueio Comercial:</strong> Pedido aguarda liberação de limite pelo setor de crédito.</p>
                )}
                {pedido.blockReason && (
                  <p className="text-xs">⚠️ <strong>Pausa Operacional:</strong> {pedido.blockReason}</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-6 font-sans">
              {pedido.produtos?.map((prod) => (
                <div key={prod.id} className="bg-slate-50 dark:bg-slate-950/20 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-4">
                  {/* Cabeçalho do Produto */}
                  <div className="flex flex-wrap justify-between items-center gap-3">
                    <div className="space-y-1">
                      <span className="bg-[#0b2f4a] text-white text-[9px] font-extrabold px-2 py-0.5 rounded uppercase">PRODUTO PRINCIPAL</span>
                  ...
                  <div className="border border-slate-150 dark:border-slate-850 bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-xs">
                    <div className="overflow-x-auto">
                      <table className="w-full text-left text-xs border-collapse">
                        <thead>
                          <tr className="bg-slate-100 dark:bg-slate-950 text-slate-500 uppercase text-[9px] font-extrabold tracking-wider border-b border-slate-200 dark:border-slate-850">
                            ...
                            <tr key={m.id} className="hover:bg-slate-50/55 dark:hover:bg-slate-950/20 font-medium">
                              ...
                              <td className="py-3 px-4 text-right">
                                <button
                                  ...
                                  className="px-2.5 py-1 bg-[#0b2f4a] hover:bg-[#123f61] text-white rounded text-[10px] font-bold transition"
                                >
                                  Ver Arte
                                </button>
                              </td>
          ...
          <div className="space-y-6 font-sans">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <button
                ...
                className="bg-[#0b2f4a] hover:bg-[#123f61] text-white text-xs font-bold px-3.5 py-2 rounded-xl transition flex items-center gap-1.5"
              >
            ...
            {pedido.blockReason ? (
              <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-250 dark:border-amber-900 rounded-3xl p-5 text-amber-800 dark:text-amber-300 flex flex-wrap justify-between items-center gap-4">
                <div className="space-y-1">
                  <h4 className="font-extrabold text-sm flex items-center gap-1.5">
              ...
              <div className="bg-slate-50 dark:bg-slate-950/20 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 flex flex-wrap items-center justify-between gap-4">
                <div className="space-y-1">
                    ...
                    {prod.modelos.map((m) => {
                      ...
                      return (
                        <div key={m.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-xs space-y-4">
                          {/* Cabeçalho do Modelo */}
                          <div className="flex flex-wrap justify-between items-center gap-3 bg-slate-100/60 dark:bg-slate-950/40 p-2.5 px-4 rounded-xl border border-slate-150 dark:border-slate-850">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="font-bold text-slate-500 dark:text-slate-400 text-xs font-mono">ID: {m.id}</span>
                              <h5 className="font-black text-sm text-[#0b2f4a] dark:text-slate-100">{m.nomeModelo}</h5>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="h-5 w-px bg-slate-250 dark:bg-slate-800"></div>
                                ...
                                {m.configImpressao.tipoNumeracao !== "SEM_NUMERACAO" && (
                                  <>
                                    <div className="flex justify-between">
                                      ...
                                      <span className="font-mono font-bold text-[#0b2f4a] dark:text-blue-400">
                              ...
                              <div className="space-y-2 text-[11px]">
                                <div className="flex items-center justify-between">
                                  <span className="text-slate-500 flex items-center gap-1">
                                    <QrCode className="h-3.5 w-3.5 text-slate-450" />
                                    QR Code:
                                  </span>
                                  <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${
                                    m.configImpressao.qrCode 
                                      ? "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400 border border-emerald-250 dark:border-emerald-900" 
                                ...
                                <div className="flex items-center justify-between">
                                  <span className="text-slate-500 flex items-center gap-1">
                                    <Barcode className="h-3.5 w-3.5 text-slate-450" />
                                    Barcode:
                                  </span>
                                  <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${
                              ...
                              <div className="pt-0.5">
                                {m.csvDadosVariaveisUrl ? (
                                  <a
                                    ...
                                    className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-blue-550/10 hover:bg-blue-500/20 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-900 font-bold transition w-full justify-center text-[10px]"
                                  >
                                    <Download className="h-3.5 w-3.5" />
                                    <span>Download CSV (VDP)</span>
                                  </a>
                                ) : (
                                  <div className="text-center py-2 text-slate-400 dark:text-slate-600 italic border border-dashed border-slate-200 dark:border-slate-850 rounded-xl bg-slate-50/20 dark:bg-slate-900/10">
                                    Sem planilha VDP
                                  </div>
                                )}
                              </div>
                          ...
                          {m.observacoesTecnicas && (
                            <div className="p-3 bg-amber-50/40 dark:bg-amber-950/10 border border-amber-100 dark:border-amber-900/40 rounded-xl text-[11px] text-slate-700 dark:text-slate-350 flex items-start gap-2">
                          ...
                          <div className="flex justify-end items-center gap-3 pt-1">
                            {m.statusArte !== "LIBERADA" ? (
                              <div className="text-[10px] font-extrabold text-red-650 dark:text-red-400 bg-red-50 dark:bg-red-950/20 p-2 px-3.5 rounded-xl border border-red-150 dark:border-red-950/40 text-center select-none flex items-center gap-1.5">
                                <AlertCircle className="h-4 w-4 shrink-0" />
                                <span>PRODUÇÃO BLOQUEADA: AGUARDANDO LIBERAÇÃO DA ARTE</span>
                              </div>
                            ) : isLocked ? (
                              <div className="text-[10px] font-extrabold text-amber-650 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20 p-2 px-3.5 rounded-xl border border-amber-150 dark:border-amber-950/40 text-center select-none flex items-center gap-1.5">
              ...
              <div className="flex flex-wrap items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3 gap-2">
                <div>
                  <h3 className={`font-black flex items-center gap-2 text-slate-900 dark:text-slate-100 ${modoGalpao ? "text-2xl" : "text-lg"}`}>
                    <Scale className="h-6 w-6 text-[#0b2f4a]" />
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl font-bold text-xs transition duration-150 flex items-center gap-1.5 border uppercase tracking-wider ${
                    modoGalpao
                      ...
                      : "bg-white hover:bg-slate-50 border-slate-200 text-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800 dark:border-slate-800 dark:text-slate-350"
                  }`}
                >
              ...
              <div className="bg-slate-100/50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 dark:bg-emerald-950/20 dark:hover:bg-emerald-950/45 dark:text-emerald-400 border border-emerald-250 dark:border-emerald-900/50 rounded-xl text-xs font-bold transition"
                  >
                    ✅ SEDEX Completo (Ok)
                  </button>
                </div>
                ...
                <div className="lg:col-span-5 space-y-6">
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 space-y-4 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div className="space-y-1">
                        ...
                        <select
                          ...
                          className="w-full h-8 px-2 rounded-lg border border-slate-205 dark:border-slate-800 bg-white dark:bg-slate-950 text-xs font-bold text-slate-850 dark:text-slate-200 focus:outline-none"
                        >
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          className="w-full h-8 px-2.5 rounded-lg border border-slate-205 dark:border-slate-800 bg-white dark:bg-slate-950 text-xs font-bold text-slate-850 dark:text-slate-200 focus:outline-none"
                        />
                      </div>
                    </div>

                    <div className="bg-slate-50 dark:bg-slate-950/30 p-3 rounded-2xl border border-slate-100 dark:border-slate-850 space-y-2 text-[11px]">
                      <div className="flex justify-between">
                        <span className="text-slate-400 font-bold uppercase text-[9px]">Cliente</span>
                        <strong className="text-slate-700 dark:text-slate-350">{pedido.clienteNome}</strong>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400 font-bold uppercase text-[9px]">Previsão de Despacho</span>
                        <strong className="text-slate-700 dark:text-slate-350">{formatDate(pedido.dataPrevistaEntrega)}</strong>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400 font-bold uppercase text-[9px]">Endereço Simplificado</span>
                        <strong className="text-slate-700 dark:text-slate-350 text-right truncate max-w-[200px]" title="Rua das Palmeiras, 456 - São Paulo / SP">
                    ...
                    <div className="space-y-1">
                      ...
                      <textarea
                        ...
                        className="w-full h-16 p-2 rounded-xl border border-slate-205 dark:border-slate-800 bg-white dark:bg-slate-955 text-xs text-slate-805 dark:text-slate-200 focus:outline-none resize-none"
                      />
                    </div>
                  </div>
                  ...
                  <div className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 space-y-4 shadow-sm ${
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-slate-50 dark:bg-slate-950/40 p-3.5 rounded-2xl border border-slate-100 dark:border-slate-850 text-center space-y-1">
                        <span className="text-[10px] font-bold text-slate-400 uppercase block tracking-wider">Peso Teórico (Estimado)</span>
                        <strong className={`font-mono block text-[#0b2f4a] dark:text-blue-400 ${modoGalpao ? "text-3xl" : "text-xl"}`}>
                      ...
                      <div className={`p-3.5 rounded-2xl border text-center space-y-1 ${
                        isPesoDivergenteReal 
                          ? "bg-red-50 dark:bg-red-950/20 border-red-250 dark:border-red-900 text-red-700 dark:text-red-400"
                    ...
                    <div className="text-xs space-y-2">
                      <div className="flex justify-between font-bold text-[11px] uppercase tracking-wider text-slate-550 dark:text-slate-400">
                        <span>Divergência:</span>
                        <span className={isPesoDivergenteReal ? "text-red-655 dark:text-red-400" : "text-emerald-650 dark:text-emerald-400"}>
                      ...
                      {isPesoDivergenteReal && totalPesoReal > 0 && (
                        <div className="bg-red-500 text-white font-black rounded-2xl p-4 text-center space-y-1.5 animate-bounce shadow-lg">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="w-12 h-7 rounded border border-slate-205 dark:border-slate-800 bg-white dark:bg-slate-950 text-xs font-bold text-center text-slate-805 dark:text-slate-200 focus:outline-none"
                        />
                      </div>
                    </div>
                              ...
                              isVolDespachado 
                                ...
                                : isVolConferido
                                  ...
                                  : isVolEmbalado
                                    ...
                                    : "bg-white dark:bg-slate-900 border-slate-205 border-slate-200 dark:border-slate-800"
                            } ${modoGalpao ? "p-5 border-2" : ""}`}
                          >
                            {/* Caixa header */}
                            <div className="flex justify-between items-center">
                              <span className={`font-black uppercase tracking-wider ${modoGalpao ? "text-base text-slate-850 dark:text-slate-100" : "text-xs text-slate-500"}`}>
                              ...
                              <select
                                ...
                                className={`h-7 px-1.5 rounded border border-slate-205 dark:border-slate-800 bg-white dark:bg-slate-950 text-[10px] font-bold focus:outline-none text-slate-850 dark:text-slate-205 cursor-pointer ${
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <div className="space-y-1">
                                ...
                                <input
                                  ...
                                  className={`w-full h-8 px-2.5 rounded-lg border border-slate-200 dark:border-slate-850 bg-white dark:bg-slate-950 text-xs font-mono font-bold focus:outline-none text-slate-850 dark:text-slate-100 ${
                              ...
                              <div className="space-y-1">
                                ...
                                <input
                                  ...
                                  className={`w-full h-8 px-2.5 rounded-lg border border-slate-205 dark:border-slate-850 bg-white dark:bg-slate-950 text-xs font-medium focus:outline-none text-slate-800 dark:text-slate-200 ${
                                    modoGalpao ? "text-sm h-10" : ""
                                  }`}
                                />
                              </div>
                            ...
                            <div className="flex justify-between items-center text-[10px] border-t border-slate-100 dark:border-slate-850 pt-2">
                              <span className="text-slate-455 font-semibold uppercase text-[9px]">Status Caixa:</span>
                              <span className={`font-black px-2 py-0.5 rounded-md text-[9px] uppercase tracking-wider ${
                                isVolDespachado 
                                  ...
                                  : isVolConferido
                                    ? "bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-450 border border-emerald-250 dark:border-emerald-900"
                                    : isVolEmbalado
                                      ? "bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-450 border border-blue-200 dark:border-blue-900"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="bg-slate-50 dark:bg-slate-950/20 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col justify-between space-y-3">
                        ...
                        <button
                          ...
                          className="w-full bg-slate-100 hover:bg-slate-200 text-slate-800 dark:bg-slate-805 dark:hover:bg-slate-700 font-bold py-2 px-3 rounded-xl transition text-[11px] uppercase tracking-wider flex items-center justify-center gap-1 border border-slate-350 dark:border-slate-700"
                        >
                      ...
                      <div className={`p-4 rounded-2xl border flex flex-col justify-between space-y-3 ${
                        isPesoDivergenteReal || !isChecklistCompleto
                          ...
                          : "bg-emerald-500/5 dark:bg-emerald-950/5 border-emerald-250 dark:border-emerald-950/40"
                      }`}>
                        <div className="space-y-1">
                          <h5 className="font-extrabold text-xs text-slate-700 dark:text-slate-350 uppercase tracking-wider">Despacho Total</h5>
                          <p className="text-[10px] text-slate-500 leading-normal">
                ...
                return (
                  <div key={msg.id} className="relative group">
                    ...
                    <div className={`absolute -left-[22px] top-2 h-2.5 w-2.5 rounded-full border bg-white dark:bg-slate-950 z-10 transition duration-150 ${
                      isCritical ? "border-red-500 bg-red-500 scale-120 shadow-xs" :
                      isSuccess ? "border-teal-500 bg-teal-500 scale-120 shadow-xs" : "border-slate-350 dark:border-slate-700 bg-white dark:bg-slate-900"
                    }`}></div>

                    {/* Ficha de Evento */}
                    <div className={`rounded-xl border p-3 text-xs space-y-1.5 transition ${
                      isCritical 
                        ? "border-red-200 bg-red-50/45 dark:border-red-950/40 dark:bg-red-950/10 text-red-950 dark:text-red-300" 
                        : isSuccess
                          ? "border-teal-150 bg-teal-50/20 dark:border-teal-950/30 dark:bg-teal-950/5 text-slate-800 dark:text-slate-350"
                          : "border-slate-150 bg-slate-50/40 dark:border-slate-800/60 dark:bg-slate-900/30 text-slate-750 dark:text-slate-350"
                    }`}>
                      {/* Event Header */}
