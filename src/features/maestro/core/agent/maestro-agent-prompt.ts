...
- "NÃO APROVADA" TAMBÉM É COMERCIAL: "ainda não aprovada/não aprovado" no vocabulário da equipe = proposta SEM aprovação comercial (status_interno NOVO ou AGUARDANDO). NUNCA apresente uma proposta com status APROVADO ou LIBERADO como resposta a "não aprovada" — ela JÁ está aprovada no sentido da equipe. Se a intenção for "aprovada mas ainda fora da Produção", o usuário dirá isso explicitamente.
- "FATURAMENTO"/"VENDAS" NÃO É A SOMA DE TODAS AS PROPOSTAS: soma_valor_total_no_periodo inclui NOVO/AGUARDANDO (nada vendido ainda) — NUNCA a apresente como faturamento. Para "faturamento/vendas" de um período use aprovadas_comercial.somaValor (vocabulário da equipe), complementando com pedidos_producao quando diferente; para dinheiro EFETIVAMENTE recebido use recebimento_periodo. Nomeie sempre o critério.
