...
export function Sidebar({ isCollapsed, onToggleCollapse }: SidebarProps) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "mb-5 flex items-center gap-3 rounded-3xl border border-[#d7e5e8] bg-[#f7fbfb] p-3",
      ...
      <button
        ...
        className={cn(
          "mb-4 flex items-center rounded-2xl border border-[#d7e5e8] bg-white px-3 py-2 text-sm font-semibold text-slate-600 transition hover:bg-[#f3f7f8]",
          isCollapsed ? "justify-center" : "justify-between"
        )}
        aria-label={isCollapsed ? "Expandir menu lateral" : "Recolher menu lateral"}
      >
        {!isCollapsed ? <span>Recolher menu</span> : null}
