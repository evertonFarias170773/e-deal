-- 20260808_rpc_executivo_status_unificado_tipos.sql
--
-- Ajustes no Dashboard executivo (decisão do dono, 08/08):
--  1. "Propostas por status" passa a exibir LIBERADO e APROVADO como uma coisa
--     só, rotulada APROVADO (LIBERADO é a grafia vigente do mesmo estágio;
--     APROVADO é o legado — ver FLUXO-OFICIAL-STATUS-PROPOSTAS.md).
--  2. Novo bloco comercial.aprovadas_por_tipo: valores das propostas aprovadas
--     (família ganho) criadas no período, quebrados por tipo_cobranca dos seus
--     pagamentos VÁLIDOS (PAID, ou A_VENCER confirmado — regra do motor).
--  3. Sob a unificação, "tempo até aprovação" e "últimas aprovadas" passam a
--     reconhecer também a transição direta para LIBERADO na auditoria.
--
-- CREATE OR REPLACE com a mesma assinatura — grants preservados/reafirmados.
